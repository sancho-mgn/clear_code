...
"""стало"""
def BastShoe(char_of_string, actual_string, undo_memory, actual_idx, len_mem):
    #избежал сравнения
    if not char_of_string:
        return actual_string
...
# ушёл от использования глобальных переменных
def App_chr(S, actual_string, undo_memory, actual_idx, len_mem):
    if actual_idx != 1:
        undo_memory = undo_memory[len_mem - actual_idx:len_mem - actual_idx + 1]
        actual_idx = 1
    # избежал сравнения
...
def Del_chr(N, actual_string, undo_memory, actual_idx, len_mem):
    if actual_idx != 1:
        undo_memory = undo_memory[len_mem - actual_idx:len_mem - actual_idx + 1]
        actual_idx = 1
...
def Return_index(i, actual_string):
    try:
...
def Undo_act(actual_string, undo_memory, actual_idx):
    # ушёл от повторения операции вычисления длины строки
    if actual_idx == len_mem:
        actual_idx = len_mem
    else:
...
def Redo_act(actual_string, undo_memory, actual_idx):
    # избежал сравнения
    if actual_idx:
        # ушёл от повторения операции вычисления длины строки
        actual_string = undo_memory[len_mem - actual_idx]
...
if __name__ == '__main__':
    actual_string = ''
    char_of_string = 'abcdevds ds www1 dd'
    actual_idx = 1
    undo_memory = []
    len_mem = len(undo_memory)
    BastShoe(char_of_string, actual_string, undo_memory, actual_idx, len_mem)    
